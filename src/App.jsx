...
const prompt =
  "Our suspect has gone north! After six days of near misses, close calls, and an alarming number of disguises, she's made her final move. Witnesses report she was last seen in a place where people willingly walk onto frozen lakes, where a shopping mall is so large it should probably have its own mayor, and where residents are so friendly they might apologize for arresting her.";<br></br> "Authorities say she purchased a new winter coat, a pair of earmuffs, and enough hand warmers to survive a small ice age. She was later overheard humming songs made famous by a legendary purple superstar before disappearing into a crowd of locals who were more than happy to give her directions—even though they were directions to the police station."; <br></br> "And remember, Gumshoes: if everyone you meet is extremely polite, talking about hockey, and offering you a casserole recipe, you're probably getting very, very close.";
...
export default function CarmenGame() {
  ...
  return (
    <div style={{ ...styles.root, animation: "flicker 8s ease-in-out infinite" }}>
      ...
      <AnimatePresence>
        {carmenHijack && <CarmenHijack phase={hijackPhase} />}
      </AnimatePresence>
...
function CarmenHijack({ phase }) {
  ...
  useEffect(() => {
    ...
    return () => { mounted.current = false; clearAll(); };
  }, []);
  ...
  useEffect(() => {
    ...
    function typeLine() {
      ...
      if (lineIdx >= CARMEN_LINES.length) {
        setDone(true);
        addTimer(() => setGlitching(true), 800);
        addTimer(() => { setGlitching(false); setEnded(true); }, 1600);
        return;
      }
      const line = CARMEN_LINES[lineIdx];
      if (line.text === "") {
        setVisibleLines(prev => [...prev, { text: "", header: false }]);
        lineIdx++;
        charIdx = 0;
        setCurrentText("");
        addTimer(typeLine, line.pause);
        return;
      }
      charIdx++;
      setCurrentText(line.text.slice(0, charIdx));
      if (charIdx >= line.text.length) {
        setVisibleLines(prev => [...prev, { text: line.text, header: line.header }]);
        setCurrentText("");
        lineIdx++;
        charIdx = 0;
        addTimer(typeLine, line.pause);
      } else {
        addTimer(typeLine, line.header ? 38 : 28 + Math.random() * 16);
      }
    }

    addTimer(typeLine, 200);
  }, [phase]);
